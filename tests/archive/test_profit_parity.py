"""Test parity between DuckDB and SQLite profit queries."""
import sys
...
def test_profit_query_parity():
    ...
    # Refresh profit MV from DuckDB (full refresh to load all data)
    ...
    duckdb_query = """
    SELECT date, COALESCE(SUM(revenue_tax_in), 0) as revenue
    ...
    # Get SQLite results from MV (destination) - bypass cache by calling unwrapped function
    # Access the underlying function from the lru_cache decorator
    sqlite_results = query_profit_trends.__wrapped__(start_date, end_date)
...
if __name__ == "__main__":
    test_profit_query_parity()
    print("Profit query parity test passed!")
